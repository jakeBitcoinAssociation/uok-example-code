...
class Interp extends struct_1.Struct {
    constructor(script, tx, nIn, stack = [], altStack = [], pc = 0, pBeginCodeHash = 0, nOpCount = 0, ifStack = [], errStr = '', flags = Interp.defaultFlags, valueBn = new bn_1.Bn(0)) {
        super({
            script,
            tx,
            nIn,
    ...
    fromJSON(json) {
        ...
        this.tx = json.tx ? new tx_1.Tx().fromJSON(json.tx) : undefined;
        return this;
    }
    /**
    ...
    fromJSONNoTx(json) {
        this.fromObject({
            script: json.script !== undefined ? new script_1.Script().fromJSON(json.script) : undefined,
            nIn: json.nIn,
        });
        this.stack = [];
    ...
    checkPubKeyEncoding(buf) {
        if ((this.flags & Interp.SCRIPT_VERIFY_STRICTENC) !== 0 && !pub_key_1.PubKey.isCompressedOrUncompressed(buf)) {
    ...
    *eval() {
    ...
    step() {
        ...
        if (opCodeNum > op_code_1.OpCode.OP_16 && ++this.nOpCount > 201) {
            this.errStr = 'SCRIPT_ERR_OP_COUNT';
            return false;
        }
        if (opCodeNum === op_code_1.OpCode.OP_LEFT ||
            opCodeNum === op_code_1.OpCode.OP_RIGHT ||
        ...
        else if (fExec || (op_code_1.OpCode.OP_IF <= opCodeNum && opCodeNum <= op_code_1.OpCode.OP_ENDIF)) {
            switch (opCodeNum) {
                //
                // Push value
                //
                ...
                case op_code_1.OpCode.OP_EQUALVERIFY:
                    // case OpCode.OP_NOTEQUAL: // use OpCode.OP_NUMNOTEQUAL
                    {
                        ...
                        const buf1 = this.stack[this.stack.length - 2];
                        const buf2 = this.stack[this.stack.length - 1];
                        const fEqual = cmp_1.cmp(buf1, buf2);
    ...
    *results(scriptSig, scriptPubKey, tx, nIn, flags, valueBn) {
        ...
        if (flags & Interp.SCRIPT_VERIFY_P2SH && scriptPubKey.isScriptHashOut()) {
            ...
            const scriptPubKey2 = new script_1.Script().fromBuffer(pubKeySerialized);
            stack.pop();
            this.initialize();
            this.fromObject({
    ...
    getDebugObject() {
        ...
        return {
            ...
            opCodeStr: this.script ? op_code_1.OpCode.fromNumber(this.script.chunks[pc].opCodeNum).toString() : 'no script found',
        };
    }
    getDebugString() {
